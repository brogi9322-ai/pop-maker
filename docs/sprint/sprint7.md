...
- 🔴 lint 오류 — `eslint.config.js`의 `globalIgnores`에 `e2e`, `playwright.config.js`, `coverage`, `playwright-report` 제외 추가: `playwright.config.js`와 `e2e/share.spec.js`에서 `process` is not defined 오류 (Node.js globals 미설정) 수정 (`eslint.config.js`)
- 🔴 lint 오류 — `e2e/share.spec.js`의 미사용 변수 `hasError` 제거: body 내용 문자열 검사 로직은 실제 assertion에 연결되지 않아 삭제 (`e2e/share.spec.js`)
- 🔴 Vitest가 `e2e/*.spec.js` 파일을 수집하여 Playwright 전용 API(`test.describe`) 오류 발생 — `vite.config.js`에 `exclude: ['e2e/**']` 추가 (`vite.config.js`)

### 다음 Sprint 개선 사항

- 🟡 모바일 가로 스크롤 overflow 버그 (`body.scrollWidth` 1009px > 뷰포트 398px) — `e2e/mobile.spec.js` TODO 주석 처리 후 Sprint 8으로 이월
- 🟡 Firebase Hosting 프리뷰 채널 배포 CI 통합 미완료 — Sprint 6에서 이월되었으나 Sprint 7에서도 미구현, Sprint 8으로 재이월
...
- `share.spec.js`의 잘못된 ID 접근 테스트에 실제 에러 상태 UI 검증 추가 (Sprint 8)
...
| `.claude/agent-memory/sprint-planner/MEMORY.md` | Sprint 7 계획 요약 추가 |
| `ROADMAP.md` | Sprint 7 진행 중 항목 추가 |
| `docs/sprint/sprint7.md` | Sprint 7 계획 문서 신규 생성 |

배포 이력: `deploy.md` 및 `docs/deploy-history/2026-03-15-sprint7.md` 참조
