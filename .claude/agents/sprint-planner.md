...
- **테스트 계획**: 순수 로직(훅/유틸)은 Vitest 단위 테스트 작성, 컴포넌트는 @testing-library/react로 렌더링/이벤트 검증
- **의존성 및 리스크**: 잠재적 블로커와 대응 방안
- **완료 기준 (Definition of Done)**: 스프린트 성공 기준 (`npm test` + `npm run build` 통과 포함)
...
- [ ] 완료 기준이 명확하게 정의되었는가? (npm test + npm run build 통과 포함)
- [ ] 신규 훅/유틸/컴포넌트에 대한 테스트 계획이 포함되었는가?
- [ ] 파일이 올바른 경로에 저장되었는가?
